...
public final class TomatoBushBlock extends SweetBerryBushBlock {
    private static final ResourceKey<LootTable> HARVEST_TOMATO_BUSH = ResourceKey.create(
        Registries.LOOT_TABLE, Identifier.fromNamespaceAndPath(Constants.MOD_ID, "harvest/tomato_bush"));
    ...
    protected ItemStack getCloneItemStack(LevelReader level, BlockPos pos, BlockState state, boolean includeData) {
        return new ItemStack(ModContent.TOMATO);
    }

    @Override
    protected InteractionResult useWithoutItem(BlockState state, Level level, BlockPos pos, Player player, BlockHitResult hitResult) {
        if (state.getValue(AGE) > 1 && level instanceof ServerLevel serverLevel) {
            Block.dropFromBlockInteractLootTable(serverLevel, HARVEST_TOMATO_BUSH, state, level.getBlockEntity(pos), null, player,
