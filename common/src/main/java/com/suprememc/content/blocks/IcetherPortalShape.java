...
public final class IcetherPortalShape {
    private static final int MAX_SIZE = 21;

    private IcetherPortalShape() {
    }
    ...
    private static boolean create(LevelAccessor level, BlockPos pos, Direction.Axis axis) {
        ...
        BlockState portal = ModContent.ICETHER_PORTAL.defaultBlockState().setValue(NetherPortalBlock.AXIS, axis);
        ...
        if (level instanceof ServerLevel serverLevel) {
            serverLevel.getDataStorage().computeIfAbsent(IcetherPortalBlock.PORTAL_CACHE).record(shape.bottomLeft);
        }
        return true;
    }
    ...
    private static Shape find(BlockGetter level, BlockPos start, Direction.Axis axis, boolean requireEmpty) {
        ...
        if (width < 2 || width > MAX_SIZE) return null;
        int height = height(level, bottomLeft, right, width, requireEmpty);
        return height >= 3 && height <= MAX_SIZE ? new Shape(bottomLeft, right, width, height) : null;
    }

    private static int edge(BlockGetter level, BlockPos start, Direction direction) {
        for (int distance = 0; distance <= MAX_SIZE; distance++) {
            BlockPos pos = start.relative(direction, distance);
            if (!interior(level.getBlockState(pos))) return level.getBlockState(pos).is(Blocks.BLUE_ICE) ? distance : 0;
            if (!level.getBlockState(pos.below()).is(Blocks.BLUE_ICE)) return 0;
        }
        return 0;
    }
    ...
    private static int height(BlockGetter level, BlockPos bottomLeft, Direction right, int width, boolean requireEmpty) {
        for (int height = 0; height < MAX_SIZE; height++) {
            if (!level.getBlockState(bottomLeft.relative(right, -1).above(height)).is(Blocks.BLUE_ICE)
                || !level.getBlockState(bottomLeft.relative(right, width).above(height)).is(Blocks.BLUE_ICE)) return height;
            for (int x = 0; x < width; x++) {
                BlockState state = level.getBlockState(bottomLeft.relative(right, x).above(height));
                if (!interior(state) || requireEmpty && state.is(ModContent.ICETHER_PORTAL)) return height;
            }
        }
        return 0;
    }
    ...
    private static boolean interior(BlockState state) {
        return state.isAir() || state.is(Blocks.FIRE) || state.is(Blocks.POWDER_SNOW) || state.is(ModContent.ICETHER_PORTAL);
    }
